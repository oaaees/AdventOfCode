        ...
        for(int i = 0; i < game.plays.size(); i++){
            for(int j = 0; j < game.plays[i].size(); j++){
                switch(game.plays[i][j].second){
                    case 'r':
                    if (red < game.plays[i][j].first) red = game.plays[i][j].first;
                    break;
                    case 'b':
                    if (blue < game.plays[i][j].first) blue = game.plays[i][j].first;
                    break;
                    case 'g':
                    if (green < game.plays[i][j].first) green = game.plays[i][j].first;
                    break;
                    default:
                    break;
                }
