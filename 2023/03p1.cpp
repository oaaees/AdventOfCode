...
int sumNeighbours(int i, int j, vector<string> &m);
...
int main (){
    ...
    for(int i = 1; i < m.size() - 1; i++){
        for(int j = 0; j < m[i].size(); j++){
            if(m[i][j] != '.' && m[i][j] < 48 || m[i][j] > 57){
                sum += sumNeighbours(i,j,m);
            }
        }
    }
...
int sumNeighbours(int i, int j, vector<string> &m){
    ...
    if(isNum(m[i - 1][j])) sum += fullNumAt(i - 1,j,m);
    if(isNum(m[i - 1][j + 1])) sum += fullNumAt(i - 1,j + 1,m);
    if(isNum(m[i][j + 1])) sum += fullNumAt(i,j + 1,m);
    if(isNum(m[i + 1][j + 1])) sum += fullNumAt(i + 1,j + 1,m);
    if(isNum(m[i + 1][j])) sum += fullNumAt(i + 1,j,m);
    if(isNum(m[i + 1][j - 1])) sum += fullNumAt(i + 1,j - 1,m);
    if(isNum(m[i][j - 1])) sum += fullNumAt(i,j - 1,m);
